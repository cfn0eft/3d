...
### 追加
- **PoseLab Studio「結果・分析」タブに対話型分析を追加**。名前に反して
  分析機能が無かったため、読み込んだ結果から**関節角度・速度・左右対称性
  (SI)** をブラウザ上で計算してグラフ＋要約統計 (平均/最小/最大/範囲/標準
  偏差) を表示する (`studio/gui/index.html`, `app_main.js`)。指標・対象関節・
  人物・**平滑化窓**・フレーム範囲などの数値を変えると即時に再計算・再描画
  される。ビューアに読み込んだデータが自動で対象になり、`JSON/CSV を開く`
  で別ファイルも直接分析できる。
- **ビューアにライブ平滑化 (平滑化窓) を追加**。数値を変えると 3D 表示が
  移動平均で即再計算され、パイプライン再実行なしでノイズの影響を確認できる
  (カメラ・フレーム・人物選択は維持)。エクスポート/分析は raw 座標を保持。
- 解析関数を 3D エンジン (`webviewer/static/engine.js`) に移植し共通化:
  `availableMetrics` / `metricSeries` / `seriesStats` / `smoothModel`
  (`analysis.py`・`kinematics.py` の角度定義・対称性指標と整合)。
  `window.PoseLab3D` から公開し、Node スモークテストにラウンドトリップ検証を
  追加 (`tests/engine_smoke.mjs`)。

### 変更
- **実行タブの左カラムを短縮 (使い勝手)**。設定が縦一列に長く積まれて実行
  ボタンが遠かったため、「処理・出力オプション」「分析・後処理 (角度/速度/
  平滑化/マスク)」を折りたたみ (`<details>`) にまとめ、**実行ボタンを下部に
  固定 (sticky)** して常に押せるようにした (`studio/gui/index.html`, `app.css`)。

## 0.9.13 (2026-06-17)

### 修正
- **出力 mp4 がブラウザ等で再生できない問題を修正**。これまで ffmpeg が
  PATH に無いと H.264 再エンコードがスキップされ、`mp4v` のままで再生
  できなかった。`imageio-ffmpeg` を依存に追加し、pip インストール時に
  静的 ffmpeg ビルドが自動取得されるようにした。`find_ffmpeg()` が
  PATH の ffmpeg → 無ければ imageio-ffmpeg 同梱バイナリの順で解決する
  (`poselab/pipeline.py`)。これにより**追加インストール無しで H.264
  (avc1) のブラウザ再生可能な mp4** が出力される。
  - Studio のシステムチェック警告・CLI の `--info` 表示も新しい解決ロジック
    に追従。`reencode_h264` は解決した ffmpeg パスを使うように変更

## 0.9.12 (2026-06-17)

### 変更
- **PoseLab Studio (Web GUI) を上部タブの 3 ページ構成にした**。1 ページに
  全カードが縦に並んで情報過多だったため、研究ワークフローに沿って分割
  (`studio/gui/index.html`, `app.css`, `app_main.js`)。
  - **実行**: ジョブ設定・システムチェック・モデルダウンロード・進捗
  - **ビューア**: 3D ビューア・プレビュー動画
  - **結果・分析**: 出力ファイル・ライブログ・履歴
  - 上部のステータス (GPU / サーバー) は常時表示。ビューアにデータを読み込む
    と自動で「ビューア」タブへ切り替わる。レスポンシブ時は 1 カラムに

## 0.9.11 (2026-06-17)

### 追加
- **デスクトップ GUI (`poselab-gui`) を Studio と機能同等にした** (`poselab/gui.py`)。
  「記録・保存」タブに **処理オプション** を追加: 平滑化手法 (移動平均 /
  メディアン / ゼロ位相 Butterworth)・窓フレーム数・カットオフ Hz・
  visibility 加重、および低信頼度マスキング閾値。Butterworth 用の fps は
  記録データのタイムスタンプから推定する。
  - エクスポートに **左右対称性 CSV** を追加 (角度・速度に加え)。マスキング
    閾値は CSV / ワイド CSV / NPZ に反映される
  - 「関節角度」タブに **歩行リズム推定** (記録データから足首の自己相関で
    cadence と 1 周期を算出) を追加
  - 一括処理の出力にも速度・対称性 CSV とマスキングを反映
  - 平滑化手法・カットオフ・加重・マスキング閾値を設定ファイルに保存

## 0.9.10 (2026-06-17)

### 追加
- **PoseLab Studio (Web GUI) に分析・処理オプションを追加**。これまで CLI
  専用だった機能をジョブ設定から指定できるようにした
  (`studio/gui/index.html`, `app_main.js`, `studio/server.py`)。
  - **分析出力**: 関節角度 CSV / 速度・加速度 CSV / 左右対称性 CSV の
    チェックボックス (MediaPipe バックエンドで出力)
  - **平滑化**: 手法 (移動平均 / メディアン / ゼロ位相 Butterworth)・窓
    フレーム数・カットオフ Hz・visibility 加重を選択 (MediaPipe)
  - **低信頼度マスキング**: visibility 閾値を指定し低信頼度座標を欠損化
    (両バックエンド対応)
  - 生成された分析 CSV は出力ファイル一覧にも表示される。MMPose (3D リフタ)
    では CLI が未対応の角度/速度/対称性/平滑化フラグを付けないようガード
    (マスキングのみ適用)
