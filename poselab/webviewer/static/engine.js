/* poselab 3D エンジン (ビューアと Pose3DStudio GUI の共通ソース)
   依存ライブラリゼロ。骨格カタログ / パーサ (parseAny) / デモ生成 /
   PoseStage レンダラ (Canvas 2D) / エクスポート関数群。
   対応形式:
   ...
   利用側:
   - ビューア: index.html が engine.js → app.js の順に読み込む
   - exe GUI : poselab-studio build が IIFE (window.PoseLab3D) で包んで連結
   - DOM 依存は PoseStage (canvas) とスナップショット保存のみ。パーサと
     エクスポートは純関数で、Node でも検証できる (tests/engine_smoke.mjs)
*/
"use strict";
